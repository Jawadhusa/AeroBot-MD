    ...
    export async function closegc() {
        ...
        try {
            ...
            if (time === '00:00' && anon === false) {
                ...
                await conn.reply(schedule, '*[System Notice]*\n\n\`\`\`مرحبا بالجميع، آسف لإزعاجكم. سيتم إغلاق نظام مجموعة الواتس اب مؤقتا بسبب وقت متأخر من الليل. نأسف للإزعاج. من فضلك خذ قسطًا من الراحة وسنواصل المحادثة صباح الغد. نشكركم على تفهمكم وتعاونكم. طاب مساؤك!.\`\`\`', null);

            } else if (time === '05:30' && anon === true) {
                await conn.groupSettingUpdate(schedule, 'not_announcement');
                await conn.delay(1000)

                await conn.reply(schedule, '*[System Notice]*\n\n\`\`\`صباح الخير يا أصدقاء! نرجو أن يكون هذا اليوم مليئًا بالحماس والسعادة. لنبدأ اليوم بمعنويات عالية ونشارك الخير في مجموعتنا على الواتساب. أتمنى لك يومًا سعيدًا ونأمل أن يكون اليوم يومًا مثمرًا وممتعًا لنا جميعًا.\`\`\`', null);

                let anon = (await conn.groupMetadata(schedule)).announce;
            } else if (time === '18:00' && anon === false) {
                await conn.groupSettingUpdate(schedule, 'announcement');
                await conn.delay(1000)
                await conn.reply(schedule, '*[System Notice]*\n\n\`\`\`مرحبا بالجميع! سيتم إغلاق نظام مجموعة الواتس اب مؤقتا بسبب دخول وقت غروب الشمس. يرجى أخذ استراحة قصيرة والاستمتاع بوقت مع العائلة أو القيام بأنشطة أخرى. سنعيد فتح نظام المجموعة هذا بعد وقت المغرب. نشكركم على تفهمكم وتعاونكم. أتمنى لك راحة جيدة!\`\`\`', null);

            } else if (time === '18:10' && anon === true) {
                await conn.groupSettingUpdate(schedule, 'not_announcement');
                await conn.delay(1000)

                await conn.reply(schedule, '*[System Notice]*\n\n\`\`\`ليلة سعيدة جميعا! تم فتح نظام مجموعة الواتساب بعد غروب الشمس. نأمل أن نكون قد قمنا جميعًا بعبادتنا بشكل جيد وحصلنا على البركات اليوم. دعونا نتشارك القصص والمعلومات والسعادة معًا في هذه المجموعة. مرحبا بكم ونتمنى أن نقضي وقتا ممتعا!\`\`\`', null);
