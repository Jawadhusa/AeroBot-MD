...
const func = (await import('./system/func/function.js'))
/**
...
`.trim(), data.jid)
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 7) {
                    this.reply(m.chat, `لقد وصل الحد الخاص بك إلى الحد الأقصى\n*قم بشراء الحد أو القسط من المالك على الفور*`, m)
...
const canWel = await new canvafy.WelcomeLeave()
    ...
    .setTitle("Welcome")
    .setDescription(`مرحبًا بك في المجموعة ${groupMetadata.subject}`)
    .setBorder("#2a2e35")
...
تم اكتشاف أن @${participant.split(`@`)[0]} قد قام بحذف الرسالة السابقة

> لإيقاف تشغيل هذه الميزة، اكتب
*.disable antidelete*
...
let msg = {
        premium: 'عذرًا، لا يمكن استخدام هذه الميزة إلا من قبل مستخدمي *Premium*',
        group: 'لا يمكن استخدام هذه الميزة إلا ضمن المجموعات',       
        private: 'لا يمكن استخدام هذه الميزة إلا ضمن الإعدادات الخاصة',       
        botAdmin: 'اجعل البوت مسؤولاً، ليتمكن من الوصول إلى المجموعة',
        ...
        premiumonly: `📢 لا يمكنك الوصول إلى الميزة ❗\nاشتر الإصدار المميز للوصول إليها عبر جهاز الكمبيوتر.\nاتصل بالمالك أدناه\nwa.me/${global.info.nomerown}`,
        groups: 'الميزة حاليًا نشطة للمجموعات فقط. الرجاء استخدام خاصية البوت في المجموعة',
        gconly: `📢 لا يمكنك الوصول إلى ميزة ❗\nالانضمام إلى مجموعة Aero Bot لتتمكن من الوصول إليها\n\n> ${global.url.sgc}`,
        game: 'الميزة *game* لم يتم تشغيلها لهذه الدردشة',
...
function ucapan() {
  ...
  let res = "إنه هذا الصباح، ألم تنم بعد؟"
  if (time >= 4) {
    res = "صباح الخير 🌄"
  }
  if (time >= 10) {
